                            ...
                            <div class="panel-body">
                                <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <c:forEach items="${homeCarouselList}" var="homeCarousel">
                                    	<tr>
                                            <td>${homeCarousel.title}</td>  
                                            <td class="logoimg"><img src="backpages/image/${homeCarousel.path}"></td>
                                           	<td>${homeCarousel.showType}</td>  
                                            <td>
                                            	<a href="/communitys/HomeCarouselController?method=deleteById&id=${homeCarousel.id}" class="btn btn-default btn-sm" onclick="return confirm('确认删除？')">删除</a>
                                            	<c:if test="${homeCarousel.showType eq '首页显示' }">
                                            		<a href="/communitys/HomeCarouselController?method=showOrNotShow&id=${homeCarousel.id}&showType=首页不显示" class="btn btn-default btn-sm" onclick="return confirm('确认取消首页显示？')">取消首页显示</a>
                                            	</c:if>
                                            	<c:if test="${homeCarousel.showType eq '首页不显示' }">
                                            		<a href="/communitys/HomeCarouselController?method=showOrNotShow&id=${homeCarousel.id}&showType=首页显示" class="btn btn-default btn-sm" onclick="return confirm('确认标记为首页显示？')">标记为首页显示</a>
                                            	</c:if>
                                            </td>
                                        </tr>
                                    </c:forEach>
