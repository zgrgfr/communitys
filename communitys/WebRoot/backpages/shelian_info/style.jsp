                                ...
                                <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <tbody id="datalist">
                                        ...
                                        <c:forEach items="${communityPresenceList}" var="communityPresence">
                                        	<tr>
	                                        	<td>${communityPresence.title}</td> 
	                                            <td>${communityPresence.author}</td>
	                                            <td>${communityPresence.time}</td>
	                                            <td>${communityPresence.type}</td>
	                                            <td>
                                                    <a href="/communitys/CommunityPresenceController?method=deleteById2&id=${communityPresence.id}" class="btn btn-default btn-sm" onclick="return confirm('确认删除？')">删除</a>
	                                            	<a href="/communitys/CommunityPresenceController?method=toUpdate2&id=${communityPresence.id}" class="btn btn-primary btn-sm" >修改</a>
	                                                <a href="/communitys/CommunityPresenceController?method=detailsView&id=${communityPresence.id}" class="btn btn-success btn-sm">查看详情</a>
	                                            	<c:if test="${sessionScope.account.role eq '超级管理员'}">
	                                            		<a href="/communitys/CommunityPresenceController?method=PassOrNotPass&id=${communityPresence.id}&type=审核通过" class="btn btn-info btn-sm" onclick="return confirm('确认审核通过？')">审核通过</a>
	                                                	<a href="/communitys/CommunityPresenceController?method=PassOrNotPass&id=${communityPresence.id}&type=审核不通过" class="btn btn-danger btn-sm" onclick="return confirm('确认审核不通过？')">审核不通过</a>
	                                            	</c:if>
	                                            </td>
                                       		</tr>
                                        </c:forEach>
