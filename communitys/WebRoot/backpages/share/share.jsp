                            ...
                            <div class="panel-body">
                                <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <c:forEach items="${resourceShareList}" var="resourceShare" >
                                    	<tr>
                                            <td>${resourceShare.title}</td> 
                                            <td>${resourceShare.communityName}</td>
                                            <td>${resourceShare.type}</td>
                                            <td>
                                                <a href="/communitys/ResourceShareController?method=deleteById&id=${resourceShare.id}" class="btn btn-default btn-sm" onclick="return confirm('确认删除？')">删除</a>
                                                <a href="/communitys/ResourceShareController?method=toUpdate&id=${resourceShare.id}" class="btn btn-primary btn-sm">修改</a>
                                                <a href="/communitys/ResourceShareController?method=detailsView&id=${resourceShare.id}" class="btn btn-success btn-sm">查看详情</a>
                                            	<c:if test="${sessionScope.account.role eq '社联管理员'}">
 													<a href="/communitys/ResourceShareController?method=passOrNotPass&id=${resourceShare.id}&type=审核通过" class="btn btn-info btn-sm" onclick="return confirm('确认审核通过？')">审核通过</a>
                                                	<a href="/communitys/ResourceShareController?method=passOrNotPass&id=${resourceShare.id}&type=审核不通过" class="btn btn-danger btn-sm" onclick="return confirm('确认审核不通过？')">审核不通过</a>
 												</c:if>
                                            </td>
                                        </tr>
                                    </c:forEach>
