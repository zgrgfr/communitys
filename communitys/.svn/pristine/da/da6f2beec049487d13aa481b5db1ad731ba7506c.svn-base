...
public class CommunityPresenceService {
	 ...
	 * @throws Exception
	 */
	public List<CommunityPresenceDto> selectAllDto() throws Exception{
		List<CommunityPresence> communityPresenceList =  communityPresenceDao.selectAll();
		...
		for(CommunityPresence communityPresence : communityPresenceList){
			// 判断社团风采和社团动态中的社团Id是否存在
			if(communityPresence.getCommunityId() != 0){
				// 判断这个社团是否存在
				if(communityService.queryById(communityPresence.getCommunityId()).getStatus() == 1){
					CommunityPresenceDto communityPresenceDto = new CommunityPresenceDto();
					communityPresenceDto.setCommunity(communityService.queryById(communityPresence.getCommunityId()));
					communityPresenceDto.setCommunityPresence(communityPresence);
					communityPresenceDtoList.add(communityPresenceDto);
				}
			}
		}
		return communityPresenceDtoList;
	}
}
