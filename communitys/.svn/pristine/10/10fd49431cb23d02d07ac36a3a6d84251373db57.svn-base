...
<body>
        <!--main content start-->
                ...
                <div class="row">
                     <div class="col-md-12">
                         <form action="/communitys/LeaguesIntroduceController" method="post">
                         	<input type="hidden" name="method" value="updateLeaguesIntroduce">
                         	<input type="hidden" name="id" value="${leaguesIntroduce.id}">
                         	<c:choose>
								<c:when test="${sessionScope.account.role eq '社联管理员'}">
                            		<input type="hidden" name="type" value="未审核">
                         			<input type="hidden" name="showType" value="首页不显示">
                            	</c:when>
                            	<c:otherwise>
                            		<input type="hidden" name="type" value="${leaguesIntroduce.type}">
                            		<input type="hidden" name="showType" value="${leaguesIntroduce.showType}">
                            	</c:otherwise>
                            </c:choose>
                         	
                         	<div class="form-group">
                                <div class="col-sm-12">
                                    <textarea id="editor" name="content" type="text/plain" style="width:980px;height:500px;">${leaguesIntroduce.content}</textarea>
                                </div>
                            </div>
                            <div class="form-group">
