                        ...
                        <div class="panel panel-default">        
                            <div class="panel-body">
                                <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <tbody id="datalist">
                                    <c:forEach items="${communityCorporatorDtoList}" var="communityCorporatorDto">
                                    	<tr>
                                            <td>${communityCorporatorDto.account.name}</td> 
                                            <td>${communityCorporatorDto.account.idNumber}</td>
                                            <td>${communityCorporatorDto.account.phone}</td>
                                            <td>${communityCorporatorDto.account.academy}</td>
                                            <td>${communityCorporatorDto.type}</td>
                                            <td>
												<a href="/communitys/CommunityCorporatorController?method=passOrNotPass&id=${communityCorporatorDto.id}&type=审核通过" class="btn btn-info btn-sm" onclick="return confirm('确认审核通过？')">审核通过</a>
                                               	<a href="/communitys/CommunityCorporatorController?method=passOrNotPass&id=${communityCorporatorDto.id}&type=审核不通过" class="btn btn-danger btn-sm" onclick="return confirm('确认审核不通过？')">审核不通过</a>
