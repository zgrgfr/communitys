...
public class CommunityController extends BaseServlet{
	 ...
	 * @return
	 */
	public String updateTopTen(HttpServletRequest req, HttpServletResponse resp){
		try{
			int topTen = ParamUtil.getInt(req, "topTen",0);
			int id = ParamUtil.getInt(req, "id",0);
			comService.updateTopTenById(topTen, id);
		}catch(Exception e){
			e.printStackTrace();
		}
		return this.listAllTopTen(req, resp);
	}
	
	/**
	 * 查看所有的十佳社团
	 * 返回的是CommunityShowDto集合
	 * @param req
	 * @param resp
	 * @return
	 */
	public String listAllTopTen(HttpServletRequest req, HttpServletResponse resp){
		try{
			// 1-是十佳社团
			List<CommunityShowDto> communityShowDtoList = comService.queryByTopTen(1);
			req.setAttribute("communityShowDtoList", communityShowDtoList);
		}catch(Exception e){
			e.printStackTrace();
		}
		return "f:backpages/top_ten/list.jsp";
	}
}
